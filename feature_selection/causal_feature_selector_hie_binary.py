...
# ----------------------------------------------------------------------
# Subsampling-safe variants: precomputed future return
# ----------------------------------------------------------------------
#
# Стандартные make_*_counterfactual_log_* функции считают future_log_ret через
# pandas shift(-horizon) внутри переданного df. Это корректно при непрерывном
# временном ряде, но ЛОМАЕТСЯ если df — это block-subsample с разрывами
# во времени между блоками: после reset_index(drop=True) shift(-H) попадает
# в следующий блок через большой временной gap, давая bogus future return.
#
# Функции ниже разделяют:
#   1) add_precomputed_future_return — считает future_log_ret_true один раз
#      на непрерывном per-symbol time series ДО любого subsampling;
#   2) make_*_from_precomputed — собирает counterfactual log из уже готового
#      future_log_ret, без shift внутри подвыборки.
#
# Использовать обязательно в BS-Stable-HIE и любых других wrappers, где HIE
# вычисляется на дискретных подвыборках строк train data.


def add_precomputed_future_return(
    df: pd.DataFrame,
    horizon: int,
    ...
    """
    Считает future_log_ret один раз на непрерывном per-symbol time series.

    Применяется ДО любого block-subsampling. После добавления колонки subsample
    может выбирать любые подмножества строк, и future_log_ret останется
    корректным, потому что он вычислен на полном непрерывном ряде.

    Returns df с добавленной колонкой `future_log_ret_col`. Строки, у которых
    future return не определён (последние `horizon` баров per symbol),
    отбрасываются.
    """
    if int(horizon) <= 0:
        raise ValueError("horizon must be positive")
    if close_col not in df.columns:
        raise ValueError(f"close column {close_col!r} missing in df")
    if symbol_col not in df.columns:
        raise ValueError(f"symbol column {symbol_col!r} missing in df")
    if timestamp_col not in df.columns:
        raise ValueError(f"timestamp column {timestamp_col!r} missing in df")

    horizon = int(horizon)
    tmp_close_col = f"_future_close_h{horizon}_tmp"

    parts = []
    for sym, g in df.groupby(symbol_col, sort=True):
        g = g.sort_values(timestamp_col).reset_index(drop=True).copy()
        g[tmp_close_col] = g[close_col].shift(-horizon)
        g[future_log_ret_col] = np.log(g[tmp_close_col] / g[close_col])
        g = g.drop(columns=[tmp_close_col])
        g = g.replace([np.inf, -np.inf], np.nan)
        g = g.dropna(subset=[future_log_ret_col]).copy()
        parts.append(g)

    if not parts:
        raise ValueError("add_precomputed_future_return: no rows survived horizon dropna.")

    return pd.concat(parts, ignore_index=True)


def make_direct_counterfactual_log_binary_success_from_precomputed(
    df: pd.DataFrame,
    feature_cols: list,
    trade_cost: float,
    alpha_out: float,
    ...
    """
    Subsampling-safe аналог make_direct_counterfactual_log_binary_success.

    Использует precomputed future_log_ret (из add_precomputed_future_return),
    НЕ делает shift(-horizon) внутри df. Это обязательно при работе с
    block-subsampled data, где shift пересекает границы блоков и даёт
    bogus future return через временной gap.

    Семантика payoff и reward идентична make_direct_counterfactual_log_binary_success:
    ...
    Аргумент `horizon` принимается только для логирования/метаданных и не
    используется в расчёте future return.
    """
    if prev_position not in (0, 1):
        raise ValueError("prev_position must be 0 or 1")
    if future_log_ret_col not in df.columns:
        raise ValueError(
            f"future_log_ret column {future_log_ret_col!r} missing in df. "
            "Did you forget add_precomputed_future_return(...) before subsampling?"
        )
    ...
    for sym, g in df.groupby("symbol", sort=True):
        ...
        # Нормализуем имя колонки future_log_ret для downstream совместимости.
        if future_log_ret_col != "future_log_ret":
            g["future_log_ret"] = g[future_log_ret_col]
        ...
        out["regime"] = regime
        out["alpha_out"] = float(alpha_out)
        out["horizon"] = int(horizon) if horizon is not None else -1
        out["trade_cost"] = float(trade_cost)
        out["side_cost_log"] = float(c_log)
        parts.append(out)
    ...
    if not parts:
        raise ValueError(
            ...
            "empty result after per-symbol filtering."
        )
